...
| `TRACE_JSONL_PATH`                   | `traces/harness.jsonl`        | Active JSONL trace file; parent dirs are created. At 10 MiB it rotates through `.1`–`.3` backups and expires the oldest. Captures full prompts/args/results — treat every retained file as sensitive and guard it at the filesystem level (trace files are not covered by `HYPHAE_API_KEY`) |

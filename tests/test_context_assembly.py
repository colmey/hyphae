...
def tool_pair(call_id: str, result: str) -> list[Message]:
    ...
    return [
        Message.assistant(
            [ToolUseBlock(id=call_id, name="srv__tool", input={"q": call_id})]
        ),
        Message.tool_results(
            [ToolResultBlock(tool_use_id=call_id, name="srv__tool", content=result)]
        ),
    ]
...
def seed_session(session, history: list[Message]) -> None:
    ...
    for msg in history:
        if msg.role == Role.USER:
            session.append_user(
                next(b.text for b in msg.content if isinstance(b, TextBlock))
            )
...
def test_token_estimators() -> None:
    ...
    check(
        estimate_text_tokens("x" * 400) == 100, "chars/4 heuristic (400 chars -> 100)"
    )
    text_msg = Message.user("What is the launch date of the probe?")
    tool_msg = Message.assistant(
        [ToolUseBlock(id="c1", name="srv__tool", input={"query": "launch date"})]
    )
    result_msg = Message.tool_results(
        [ToolResultBlock(tool_use_id="c1", name="srv__tool", content="date=2031-06-01")]
    )
    for label, msg in [
        ("text", text_msg),
        ("tool-use", tool_msg),
        ("tool-result", result_msg),
    ]:
...
def test_context_budget_math() -> None:
    ...
    check(
        ContextBudget(context_window=100, max_output_tokens=200).input_budget == 0,
...
async def test_compaction_strategy() -> None:
    ...
    small = [Message.user("hi")]
    llm = RecordingLLM(
        [], summary_text="- launch narrowed to 2031\n- landing site pending"
    )
    ...
    check(
        assembled.messages[0] == history[0], "first user message (task header) verbatim"
    )
    summary_texts = [
        b.text
    ...
    check(
        len(summary_texts) == 1 and "2031" in summary_texts[0],
    ...
    check(
        assembled.estimated_input_tokens <= budget.input_budget,
...
async def test_loop_uses_compacted_view_without_mutating_session_history() -> None:
    ...
    check(
        any(
            isinstance(b, TextBlock) and b.text.startswith(_SUMMARY_HEADER)
            for m in sent
            for b in m.content
        ),
        "compacted view contains the summary",
    )
    check(protocol_ok(sent), "compacted view is protocol-safe")
    check(
        session.messages[: len(before)] == before,
        "session history untouched by compaction (view-only)",
    )
    check(
        len(session.messages) == len(before) + 1
...
async def test_agent_token_cap_falls_back_to_estimator() -> None:
    ...
    llm = RecordingLLM(
        [text_response("a zero-usage answer that is long enough to count")]
    )
    store = InMemorySessionStore()
    session = await store.create()

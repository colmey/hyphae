...
class ScriptedMCP:
    ...
    def __init__(self, script: list[tuple[str, bool]] | None = None,
    ...
    def get_tools_for_llm(self) -> list[dict[str, Any]]:
        return [{"name": "srv__tool", "description": "test tool", "input_schema": self._schema}]
...
def text_response(text: str) -> AssistantMessage:
    return AssistantMessage(content=[TextBlock(text=text)], stop_reason="end_turn", usage=Usage())


def text_and_tool_response(text: str, args: dict[str, Any], *, call_id: str = "call_1",
                           usage: Usage | None = None) -> AssistantMessage:
    return AssistantMessage(
        content=[TextBlock(text=text), ToolUseBlock(id=call_id, name="srv__tool", input=args)],
        stop_reason="tool_use", usage=usage or Usage(),
    )


def tool_call_response(args: dict[str, Any], *, call_id: str = "call_1") -> AssistantMessage:
    return AssistantMessage(
...
async def test_final_answer_crossing_token_cap_reports_budget_exceeded() -> None:
    ...
    events = await collect("final-answer token cap", llm, mcp,
                           max_run_tokens=30, max_iterations=10)
...
async def test_final_answer_crossing_deadline_is_not_streamed() -> None:
    ...
    events = await collect("final-answer deadline cap", llm, mcp,
                           max_run_seconds=0.03, max_iterations=10)
    check(done_reason(events) == "deadline_exceeded",
...
async def test_slow_tool_exhausting_deadline_ends_run() -> None:
    ...
    events = await collect("slow-tool deadline cap", llm, mcp,
                           max_run_seconds=0.03, tool_timeout_seconds=1.0,
                           max_iterations=10)
    check(done_reason(events) == "deadline_exceeded",
          "slow tool crossing wall clock reports deadline_exceeded")
    check(llm.calls == 1, f"no second LLM call after slow-tool deadline (got {llm.calls})")
...
async def test_invalid_tool_arguments_are_not_dispatched() -> None:
    ...
    check("'q'" in trs[0].content or '"q"' in trs[0].content, "error names the offending field")
    check(mcp.call_count == 1, f"mcp.call_tool ran only for the valid call (got {mcp.call_count})")
...
async def test_consecutive_tool_failures_abort_no_progress() -> None:
    ...
    events = await collect("no-progress abort", llm, mcp,
                           abort_after_consecutive_tool_failures=5, max_iterations=20)
...
async def test_success_resets_tool_failure_counter() -> None:
    llm = ScriptedLLM([
        ...
        tool_call_response({"q": "3"}, call_id="call_3"),  # succeeds, resets the streak
    ...
    events = await collect("no-progress counter reset by a success", llm, mcp,
                           abort_after_consecutive_tool_failures=3, max_iterations=20)
...
async def test_mid_batch_abort_preserves_tool_call_result_pairing() -> None:
    ...
    check(len(tool_calls(events)) == 3, f"all 3 tool calls surfaced (got {len(tool_calls(events))})")
    check(len(tool_results(events)) == 3,
          f"all 3 tool results emitted (got {len(tool_results(events))})")
    check(len(saved_tool_results) == 3,

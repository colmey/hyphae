...
class FakeMCP:
    ...
    async def call_tool(self, name: str, arguments: dict[str, Any]) -> ToolCallResult:
        ...
        return ToolCallResult(content=f"lookup result for {arguments.get('query')}", is_error=False)


def action_text(query: str = "example") -> str:
    return (
        "```json\n"
        f'{{"tool":"srv__lookup","arguments":{{"query":"{query}"}}}}\n'
        "```"
    )
...
def test_parser() -> None:
    ...
    check(parsed.action is not None and parsed.action.name == "srv__lookup", "action name parsed")
    check(
        parsed.action is not None and parsed.action.arguments == {"query": "alpha"},
        "action arguments parsed",
    )

    parsed = parse_prompted_action('{"name":"srv__lookup","input":{"query":"beta"}}', allowed)
    check(
    ...
    check(parsed.action is None and parsed.error is None, "ordinary prose is final text")
    check(parsed.text == "Here is the final answer.", "final prose is preserved")

    parsed = parse_prompted_action('```json\n{"tool":"srv__lookup","arguments":}\n```', allowed)
    check(parsed.action is None and parsed.error is not None, "malformed JSON is an error")

    parsed = parse_prompted_action('{"tool":"srv__missing","arguments":{}}', allowed)
    check(parsed.action is None and parsed.error is not None, "unknown tool is an error")
    ...
    check(parsed.action is None and parsed.error is not None, "non-dict args are an error")
...
async def test_wrapper_basic() -> None:
    ...
    check("base system" in (inner.systems_seen[0] or ""), "original system is preserved")
    check("Available tools:" in (inner.systems_seen[0] or ""), "prompted tool instructions are in system")
    check(len(response.content) == 1 and isinstance(response.content[0], ToolUseBlock), "wrapper returns one ToolUseBlock")
    tool_use = response.content[0]
    check(isinstance(tool_use, ToolUseBlock) and tool_use.name == "srv__lookup", "ToolUseBlock name normalized")
    check(isinstance(tool_use, ToolUseBlock) and tool_use.input == {"query": "gamma"}, "ToolUseBlock input normalized")
...
async def test_passthroughs() -> None:
    ...
    check(response.text_blocks()[0].text == "structured", "response_schema call passes through")
    check(inner.tools_seen == [[TOOL]], "response_schema passthrough keeps native tools argument")
    check(inner.response_schemas_seen == [schema], "response_schema reaches inner client")

    history = [
        Message.user("lookup"),
        Message.assistant([ToolUseBlock(id="prompted_old", name="srv__lookup", input={"query": "old"})]),
    ...
    check(response.text_blocks()[0].text == "final from history", "final prose passes through")
...
async def test_repair() -> None:
    ...
    wrapper = PromptedToolLLMClient(inner)
    response = await wrapper.complete(messages=[Message.user("lookup repaired")], tools=[TOOL])
    check(inner.calls == 2, "malformed JSON triggers one repair call")
    repair_text = "".join(
        block.text for block in inner.messages_seen[1][-1].content if isinstance(block, TextBlock)
    )
    check("invalid JSON tool action" in repair_text, "repair prompt includes parse error")
    check(response.tool_uses()[0].input == {"query": "repaired"}, "repair action becomes ToolUseBlock")
    check(response.usage is not None and response.usage.total_tokens == 5, "usage from repair calls is combined")
    ...
    wrapper = PromptedToolLLMClient(inner)
    response = await wrapper.complete(messages=[Message.user("lookup fail")], tools=[TOOL])
...
async def test_semantic_errors() -> None:
    ...
    wrapper = PromptedToolLLMClient(inner)
    response = await wrapper.complete(messages=[Message.user("lookup bad")], tools=[TOOL])
...
async def test_drop_in_loop() -> None:
    ...
    check(mcp.calls == [("srv__lookup", {"query": "phase6"})], "MCP dispatched parsed args")
    check(len(tool_results) == 1 and not tool_results[0].is_error, "loop emitted successful ToolResultEvent")
    check("Final answer" in "".join(e.text for e in text_events), "loop emitted final TextEvent")
...
async def test_factory_selection() -> None:
    ...
    try:
        ...
        check(not isinstance(native, PromptedToolLLMClient), "omitted supports_native_tools stays native")
        ...
        check(not isinstance(native_true, PromptedToolLLMClient), "true supports_native_tools stays native")
        ...
        check(isinstance(prompted, PromptedToolLLMClient), "false supports_native_tools wraps client")
        check(isinstance(prompted.inner, ScriptedLLM), "wrapper contains base fake client")
    finally:
